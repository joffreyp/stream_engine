...
def parse_qartod_dict(qartod_dict):
    ...
    refdes = qartod_dict.get("Reference Designator")  # No examples in current qc-lookup
    if subsite:
        if subsite.startswith("#"):  # commented out
            return None
        ...
        refdes = qartod_dict.get("refdes")  # No examples in current qc-lookup
        subsite, node, sensor = refdes.split("-", 2)

    stream = qartod_dict.get("stream")
    if not stream:
        # No examples of this element in current qc-lookup
        stream = qartod_dict.get("Stream Identifier")

    parameters = qartod_dict.get("parameters")
    if not parameters:
        # CTD Gross Range Lookup
        param = qartod_dict.get("Parameter Name")  # No examples in current qc-lookup
        if not param:
            # CGSN CTD Gross Range Lookup
    ...
    if not qcconfig and climatologyTable:
        ...
        if suspect_span_string:
            suspect_span = [
                parse_number(x) for x in suspect_span_string[1:-1].split(", ")
            ]
...
def parse_qartod_file(filepath):
    ...
    with open(filepath, mode="r") as csvfile:
        ...
        for row in reader:
            qartod_entry = parse_qartod_dict(row)
            if qartod_entry is not None:
                qartod_list.append(qartod_entry)
    return qartod_list
...
def parse_climatology_table(filepath, param_dict):
    ...
    with open(filepath, mode="r") as csvfile:
        ...
        includeZSpan = True
        if param_dict and (
            param_dict["zinp"] == "null" or param_dict["zinp"] == "None"
        ):
            includeZSpan = False
        ...
        for row in reader:
            if row:
                ...
                for i in range(1, len(time_bins)):
                    if includeZSpan:
                        data = (
                            "{'tspan': %s, 'zspan': %s, 'vspan': %s, 'period': 'month'}"
                            % (time_bins[i], pressure_bin, row[i])
                        )
                    else:
                        data = "{'tspan': %s, 'vspan': %s, 'period': 'month'}" % (
                            time_bins[i],
                            row[i],
                        )
                    configs.append(data)
...
def insert_qartod_records(qartod_list, hostname=None, username=None, password=None):
    host = hostname or raw_input(
        "Connecting to PostgreSQL to insert QARTOD records...\nhost: "
    )
    username = username or raw_input(
        "Connecting to PostgreSQL to insert QARTOD records...\nusername: "
    )
            ...
            if parsed_qartod_dict:
                with connection:
                    with connection.cursor() as cursor:
                        ...
                        fields = sql.SQL(", ").join(
                            map(sql.Identifier, parsed_qartod_dict.keys())
                        )
                        values = sql.SQL(", ").join(
                            map(sql.Placeholder, parsed_qartod_dict.keys())
                        )
                        cursor.execute(
                            sql.SQL("INSERT INTO qartod_tests ({}) VALUES ({})").format(
                                fields, values
                            ),
                            parsed_qartod_dict,
                        )
...
def main(filepath=None):
    if filepath is None:
        parser = argparse.ArgumentParser(
            description="Parse a CSV indicating QARTOD test records."
        )
        parser.add_argument(
            "file", type=str, help="filepath for the QARTOD test CSV file to parse"
        )
        args = parser.parse_args()
        filepath = args.file
    data = parse_qartod_file(filepath)
